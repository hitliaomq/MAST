    ...
    for s,c,m,u in atomlist:
        if c > 0:
            for i in range(c):
             ...
             #   pos = [r*u[0]+size/2.0, r*u[1]+size/2.0, r*u[2]/2.0+size/2.0]
                pos = [a*r*u[0]+size/2.0, b*r*u[1]+size/2.0, c*r*u[2]+size/2.0]
             #   seed = False
             #   while seed == False:
             #        d = [random.uniform(-1.0,1.0) for j in range(3)]
             #        if numpy.dot(d,d) < 1.0:
             #           seed = True
             #           pos = [a*d[0]+size/2.0, b*d[1]+size/2.0, c*d[2]+size/2.0]  
