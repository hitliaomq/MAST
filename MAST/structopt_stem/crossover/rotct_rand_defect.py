...
from MAST.structopt_stem.tools.position_average import position_average
from MAST.structopt_stem.tools.shift_atoms import shift_atoms
from MAST.structopt_stem.inp_out import write_xyz
...
def rotct_rand_defect(ind1, ind2, Optimizer):
    ...
    if 'CX' in Optimizer.debug:
        ...
        debug = False
    Optimizer.output.write('Random Rotate Cut/Splice Cx for defects between individual '+repr(ind1.index)+' and individual '+repr(ind2.index)+'\n')
    ...
    if len(group1) != 0:
        ...
        for sym,c,m,u in Optimizer.atomlist:
            ...
            Optimizer.output.write('CX RANDROTCT_Defect: Individual 1 contains '+repr(nc)+' '+repr(sym)+' atoms\n')
            nc=len([atm for atm in indi2 if atm.symbol==sym])
            Optimizer.output.write('CX RANDROTCT_Defect: Individual 2 contains '+repr(nc)+' '+repr(sym)+' atoms\n')
        if Optimizer.forcing !='Concentration':
        ...
        if Optimizer.structure=='Defect':
            ...
            for sym,c,m,u in Optimizer.atomlist:
                ...
                Optimizer.output.write('CX RANDROTCT_Defect: Defect 1 configuration contains '+repr(nc)+' '+repr(sym)+' atoms\n')
                nc=len([atm for atm in solid2 if atm.symbol==sym])
                Optimizer.output.write('CX RANDROTCT_Defect: Defect 2 configuration contains '+repr(nc)+' '+repr(sym)+' atoms\n')
